...
import {
  ...
  saveMetricSnapshot,
  saveMetricSnapshots,
  saveTechnicalReadinessCheck,
  savePromptMonitor,
  savePromptSnapshot,
  saveResearchQuestion,
  saveTopicCluster,
} from "./data/intelligenceStore";
import { CITATION_PROTOCOL, GEO_READINESS_CHECKS, READINESS_STATUS } from "./data/geoReadiness";
...
const emptySnapshot = { checkedAt: today(), answerSummary: "", brandMentioned: false, domainCited: false, citedDomains: "", evidenceUrl: "", reviewerNote: "", citationType: "none", sourceChecked: false, sourceRelevant: false, referralObserved: false, reviewDecision: "not_checked" };
const emptyReadiness = { status: "pending", note: "", reviewedAt: today() };
...
export default function IntelligenceView({ workspace, data, onRefresh }) {
  ...
  const readinessByKey = useMemo(() => new Map(data.readinessChecks.map(item => [item.check_key, item])), [data.readinessChecks]);
  const nextReadinessCheck = useMemo(() => GEO_READINESS_CHECKS.find(check => readinessByKey.get(check.key)?.status !== "passed") || null, [readinessByKey]);
  const readinessPassedCount = useMemo(() => GEO_READINESS_CHECKS.filter(check => readinessByKey.get(check.key)?.status === "passed").length, [readinessByKey]);
  ...
  useEffect(() => {
    if (!nextReadinessCheck) return;
    const stored = readinessByKey.get(nextReadinessCheck.key);
    setReadinessForm({
      status: stored?.status || "pending",
      note: stored?.note || "",
      reviewedAt: stored?.reviewed_at || today(),
    });
  }, [nextReadinessCheck, readinessByKey]);
  ...
  const focus = useMemo(() => {
    ...
    if (nextReadinessCheck) return { type: "readiness", title: `Technische Basis prüfen: ${nextReadinessCheck.title}`, text: nextReadinessCheck.prompt, action: "Readiness öffnen" };
  ...
  }, [activeMonitors, coveredMonitors, data.claims, data.metrics.length, data.profile?.domain, nextReadinessCheck, rankedQuestions, sourceCountByClaim, sourcesDue]);
  ...
  const verifiedCitationRate = data.snapshots.length ? Math.round((data.snapshots.filter(item => item.domain_cited && item.source_checked && item.source_relevant && item.review_decision === "confirmed").length / data.snapshots.length) * 100) : 0;
  ...
  return <section className="view-stack intelligence-view">
    ...
    <section className="intelligence-score-grid"><article><p className="eyebrow">MESSBASIS</p><strong>{data.metrics.length}</strong><span>gespeicherte Werte</span></article><article><p className="eyebrow">TECHNIK</p><strong>{readinessPassedCount}/{GEO_READINESS_CHECKS.length}</strong><span>Readiness-Schritte geprüft</span></article><article><p className="eyebrow">VERIFIZIERTE ZITATE</p><strong>{data.snapshots.length ? `${verifiedCitationRate}%` : "–"}</strong><span>in dokumentierten Stichproben</span></article></section>

    <div className="intelligence-tabs" role="tablist" aria-label="GEO Intelligence Bereiche">{[["focus", "Übersicht"], ["readiness", "Readiness"], ["measurement", "Messbasis"], ["evidence", "Evidenz"], ["monitor", "GEO-Monitor"]].map(([id, label]) => <button key={id} role="tab" aria-selected={section === id} className={section === id ? "active" : ""} onClick={() => setSection(id)}>{label}</button>)}</div>
    ...
    {section === "readiness" && <section className="intelligence-section"><div className="section-copy"><p className="eyebrow">STUFE 0 · TECHNISCHE READINESS</p><h2>Erst auffindbar und verständlich. Dann Wirkung messen.</h2><p>Immer nur einen technischen Grundsatz prüfen. Ein grüner Check ist eine dokumentierte Grundlage – keine Zitationsgarantie.</p></div><div className="intelligence-two-column"><form className="intelligence-form readiness-focus-form" onSubmit={event => { event.preventDefault(); if (!nextReadinessCheck) return; submit("readiness", () => saveTechnicalReadinessCheck({ workspaceId: workspace.id, check: { key: nextReadinessCheck.key, ...readinessForm } }), `„${nextReadinessCheck.title}“ wurde gespeichert.`); }}><div className="form-heading"><div><p className="eyebrow">DEIN NÄCHSTER TECHNIK-CHECK</p><h3>{nextReadinessCheck ? nextReadinessCheck.title : "Technische Basis vollständig geprüft"}</h3></div><span>{nextReadinessCheck ? "1 Schritt" : "Bereit"}</span></div>{nextReadinessCheck ? <><p className="readiness-question">{nextReadinessCheck.prompt}</p><p className="readiness-help">{nextReadinessCheck.help}</p><div className="form-grid two"><label>Status<select value={readinessForm.status} onChange={event => setReadinessForm(current => ({ ...current, status: event.target.value }))}>{Object.entries(READINESS_STATUS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Geprüft am<input type="date" value={readinessForm.reviewedAt} onChange={event => setReadinessForm(current => ({ ...current, reviewedAt: event.target.value }))} /></label></div><label>Kurze Notiz<textarea rows="3" value={readinessForm.note} onChange={event => setReadinessForm(current => ({ ...current, note: event.target.value }))} placeholder="Was wurde konkret geprüft oder was blockiert noch?" /></label><button className="primary-button" disabled={busy === "readiness"}>{busy === "readiness" ? "Wird gespeichert …" : "Check speichern"}</button></> : <p className="empty-copy">Alle Grundlagen sind dokumentiert. Prüfe bei technischen Änderungen oder neuen Zielseiten erneut.</p>}</form><article className="intelligence-panel readiness-list"><div className="panel-heading"><div><p className="eyebrow">CHECKLISTE</p><h2>Technische Grundlage</h2></div><span>{readinessPassedCount}/{GEO_READINESS_CHECKS.length}</span></div>{GEO_READINESS_CHECKS.map(check => { const stored = readinessByKey.get(check.key); return <article key={check.key}><div><b>{check.title}</b><small>{stored?.note || check.help}</small></div><em className={stored?.status === "blocked" ? "amber" : ""}>{READINESS_STATUS[stored?.status || "pending"]}</em></article>; })}</article></div></section>}
    ...
    {section === "monitor" && <section className="intelligence-section">
      <div className="section-copy"><p className="eyebrow">STUFE C · GEO-MONITORING</p><h2>Definierte Prompts statt zufälliger Beispiele.</h2><p>Dokumentiere nur Antworten, die du selbst geprüft hast. Das ist eine Stichprobe für Trends – keine Garantie für Rankings oder KI-Zitate.</p></div>
      <article className="citation-protocol-card"><div><p className="eyebrow">ZITATIONSPROTOKOLL</p><h3>Ein sichtbares Signal ist erst nach der Quellenprüfung belastbar.</h3></div><div className="citation-protocol-list">{CITATION_PROTOCOL.map(([number, title, text]) => <article key={number}><b>{number}</b><div><strong>{title}</strong><span>{text}</span></div></article>)}</div></article>
      <div className="intelligence-two-column">
        <form className="intelligence-form" onSubmit={event => { event.preventDefault(); submit("monitor", () => savePromptMonitor({ workspaceId: workspace.id, monitor: monitorForm }), "Prompt für die wiederkehrende Prüfung gespeichert.", () => setMonitorForm(emptyMonitor)); }}><div className="form-heading"><div><p className="eyebrow">PROMPT-BIBLIOTHEK</p><h3>Eine echte Zielgruppenfrage.</h3></div></div><label>Prompt<input required value={monitorForm.promptText} onChange={event => setMonitorForm(current => ({ ...current, promptText: event.target.value }))} placeholder="Welche Tools helfen bei GEO?" /></label><div className="form-grid two"><label>Zugehörige Nutzerfrage<select value={monitorForm.questionId} onChange={event => setMonitorForm(current => ({ ...current, questionId: event.target.value }))}><option value="">Optional</option>{data.questions.map(question => <option value={question.id} key={question.id}>{question.question}</option>)}</select></label><label>Prüfsystem<input value={monitorForm.systemName} onChange={event => setMonitorForm(current => ({ ...current, systemName: event.target.value }))} /></label><label>Sprache<input value={monitorForm.languageCode} onChange={event => setMonitorForm(current => ({ ...current, languageCode: event.target.value }))} /></label><label>Priorität<select value={monitorForm.priority} onChange={event => setMonitorForm(current => ({ ...current, priority: event.target.value }))}>{[1,2,3,4,5].map(value => <option key={value} value={value}>{value} von 5</option>)}</select></label></div><button className="quiet-button" disabled={busy === "monitor"}>{busy === "monitor" ? "Speichern …" : "Prompt anlegen"}</button></form>
        <form className="intelligence-form" onSubmit={event => { event.preventDefault(); if (!selectedMonitor) return setNotice({ type: "error", text: "Lege zuerst einen Prompt an." }); if (snapshotForm.reviewDecision === "confirmed" && (!snapshotForm.sourceChecked || !snapshotForm.sourceRelevant)) return setNotice({ type: "error", text: "Für „Zitation bestätigt“ musst du die zitierte Quelle selbst öffnen und als passend prüfen." }); submit("snapshot", () => savePromptSnapshot({ monitorId: selectedMonitor.id, snapshot: snapshotForm }), "Prüfergebnis mit Zitationsstatus dokumentiert.", () => setSnapshotForm(emptySnapshot)); }}><div className="form-heading"><div><p className="eyebrow">MANUELLE PRÜFUNG</p><h3>Was war tatsächlich sichtbar?</h3></div></div><label>Prompt<select value={selectedMonitor?.id || ""} onChange={event => setSelectedMonitorId(event.target.value)}><option value="">Prompt auswählen …</option>{activeMonitors.map(monitor => <option key={monitor.id} value={monitor.id}>{monitor.prompt_text}</option>)}</select></label><div className="form-grid two"><label>Prüfdatum<input type="date" value={snapshotForm.checkedAt} onChange={event => setSnapshotForm(current => ({ ...current, checkedAt: event.target.value }))} /></label><label>Wurde die Marke genannt?<select value={snapshotForm.brandMentioned ? "yes" : "no"} onChange={event => setSnapshotForm(current => ({ ...current, brandMentioned: event.target.value === "yes" }))}><option value="no">Nein</option><option value="yes">Ja</option></select></label><label>Wurde geo-tool.com zitiert?<select value={snapshotForm.domainCited ? "yes" : "no"} onChange={event => setSnapshotForm(current => ({ ...current, domainCited: event.target.value === "yes" }))}><option value="no">Nein</option><option value="yes">Ja</option></select></label><label>Sichtbares Signal<select value={snapshotForm.citationType} onChange={event => setSnapshotForm(current => ({ ...current, citationType: event.target.value }))}><option value="none">Kein Zitat</option><option value="brand_mention">Nur Markennennung</option><option value="domain_citation">Domain zitiert</option><option value="source_citation">Quelle zitiert</option></select></label></div><label>Antwort kurz zusammenfassen<textarea required rows="3" value={snapshotForm.answerSummary} onChange={event => setSnapshotForm(current => ({ ...current, answerSummary: event.target.value }))} placeholder="Was wurde in der Antwort gesagt?" /></label><label>Genannte Domains, durch Komma trennen<input value={snapshotForm.citedDomains} onChange={event => setSnapshotForm(current => ({ ...current, citedDomains: event.target.value }))} /></label><label>Öffentlicher Nachweis, falls vorhanden<input type="url" value={snapshotForm.evidenceUrl} onChange={event => setSnapshotForm(current => ({ ...current, evidenceUrl: event.target.value }))} placeholder="https://…" /></label><div className="citation-check-grid"><label className={snapshotForm.sourceChecked ? "checked" : ""}><input type="checkbox" checked={snapshotForm.sourceChecked} onChange={event => setSnapshotForm(current => ({ ...current, sourceChecked: event.target.checked, sourceRelevant: event.target.checked ? current.sourceRelevant : false }))} /><span><b>Quelle selbst geöffnet</b>Die zitierte URL wurde nicht nur aus der KI-Antwort übernommen.</span></label><label className={snapshotForm.sourceRelevant ? "checked" : ""}><input type="checkbox" checked={snapshotForm.sourceRelevant} disabled={!snapshotForm.sourceChecked} onChange={event => setSnapshotForm(current => ({ ...current, sourceRelevant: event.target.checked }))} /><span><b>Quelle passt zur Aussage</b>Die Originalquelle stützt den dokumentierten Punkt tatsächlich.</span></label><label className={snapshotForm.referralObserved ? "checked" : ""}><input type="checkbox" checked={snapshotForm.referralObserved} onChange={event => setSnapshotForm(current => ({ ...current, referralObserved: event.target.checked }))} /><span><b>Referral separat sichtbar</b>Ein Messwert oder UTM-Referral wurde unabhängig dokumentiert.</span></label></div><label>Zitationsentscheidung<select value={snapshotForm.reviewDecision} onChange={event => setSnapshotForm(current => ({ ...current, reviewDecision: event.target.value }))}><option value="not_checked">Noch nicht geprüft</option><option value="confirmed">Zitation bestätigt</option><option value="needs_review">Zitat erneut prüfen</option><option value="not_relevant">Keine relevante Zitation</option></select></label><label>Notiz<textarea rows="2" value={snapshotForm.reviewerNote} onChange={event => setSnapshotForm(current => ({ ...current, reviewerNote: event.target.value }))} /></label><button className="primary-button" disabled={busy === "snapshot"}>{busy === "snapshot" ? "Wird gespeichert …" : "Prüfergebnis sichern"}</button></form>
      </div>
      <article className="intelligence-panel"><p className="eyebrow">AKTIVE PROMPTS</p>{activeMonitors.length ? <div className="intelligence-list">{activeMonitors.map(monitor => { const latest = data.snapshots.find(snapshot => snapshot.monitor_id === monitor.id); const verified = latest?.domain_cited && latest?.source_checked && latest?.source_relevant && latest?.review_decision === "confirmed"; return <article className={selectedMonitor?.id === monitor.id ? "selected" : ""} key={monitor.id}><button onClick={() => setSelectedMonitorId(monitor.id)}><b>{monitor.prompt_text}</b><small>{monitor.system_name} · {latest ? `letzte Prüfung: ${formatDate(latest.checked_at.slice(0, 10))}` : "noch nicht geprüft"}</small></button><em>{verified ? "Verifiziert" : latest?.domain_cited ? "Zitat prüfen" : latest ? "Geprüft" : "Offen"}</em></article>; })}</div> : <p className="empty-copy">Lege einen Prompt an. Er wird erst nach manueller Prüfung als Signal gezählt.</p>}</article>
      {selectedMonitor && selectedMonitorSnapshots.length > 0 && <article className="intelligence-panel"><p className="eyebrow">LETZTE PRÜFUNGEN</p><div className="intelligence-list">{selectedMonitorSnapshots.map(snapshot => { const verified = snapshot.domain_cited && snapshot.source_checked && snapshot.source_relevant && snapshot.review_decision === "confirmed"; return <article key={snapshot.id}><div><b>{verified ? "Domain-Zitation bestätigt" : snapshot.domain_cited ? "Domain-Zitation noch prüfen" : "Keine Domain-Zitation dokumentiert"}</b><small>{formatDate(snapshot.checked_at.slice(0, 10))} · {snapshot.answer_summary}</small></div></article>; })}</div></article>}
    </section>}
  </section>;
}
